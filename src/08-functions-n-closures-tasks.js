...
function getPolynom(...args) {
  ...
  return (x) => {
    const res = coefs.reduce((acc, it, idx) => it * x ** (idx + 1) + acc, last);
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    logFunc(`${func.name}(${a.substring(1, a.length - 1)}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${a.substring(1, a.length - 1)}) ends`);
    return res;
  };
}
...
function partialUsingArguments(fn, ...args) {
  const foo = (...args2) => (args2.length >= fn.length
    ? fn(...args2)
    : (...args3) => foo(...args2, ...args3));

  return args.length === fn.length ? () => fn(...args) : foo(...args);
}
